...
/* Mutex to protect outbound connection from multiple senders */
static pthread_mutex_t call_send_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static CK_RV call_write(unsigned char *data, size_t len)
{
	int r;
	...
	while (len > 0) {
        ...
        r = send(call_socket, (void *)data, len, 0);
...
static CK_RV call_read(unsigned char *data, size_t len)
{
	int r;
	...
	while (len > 0) {
        ...
        r = recv(call_socket, (void *)data, len, 0);
...
static CK_RV call_connect()
{
	struct sockaddr_un addr;
	int sock;

	assert(call_socket == -1);
...
static CK_RV call_create(CallState ** ret)
{
	...
	debug(("call_create: start"));
	assert(ret);

	/* Initialize new call state entry */
	...
	if (pthread_mutex_init(&cs->wait_mutex, NULL) != 0)
		goto cleanup;
	if (pthread_cond_init(&cs->wait_cond, NULL) != 0)
		goto cleanup;
	if (pthread_mutex_lock(&cs->wait_mutex) != 0)
		goto cleanup;
...
static CK_RV call_prepare(CallState * cs, int call_id)
{
	...
	if (!cs->req) {
		...
		if (!cs->req) {
			...
			debug(("allocated request buffer"));
	}
...
static CK_RV call_send_recv(CallState * cs)
{
	...
	egg_buffer_encode_uint32(buf+4, cs->req->buffer.len);
	pthread_mutex_lock(&call_send_mutex);
	ret = call_write(buf, 8);
	if (ret == CKR_OK)
...
static CK_RV rpc_C_Initialize(CK_VOID_PTR init_args)
{
	...
	if (pthread_attr_init(&tattr) != 0) {
		warning(("Cannot initialize thread attributes"));
		return CKR_HOST_MEMORY;
	}
	...
	debug(("Own AppID %08x-%08x\n", (uint32_t) (pkcs11_app_id >> 32),
		    (uint32_t) pkcs11_app_id));
	...
	if ((rc = pthread_create(&rthread_id, &tattr, receiver_thread, NULL)) != 0) {
		warning(("Cannot create receiver thread", rc));
		ret = CKR_HOST_MEMORY;
	}
